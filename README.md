...
**Status: discovery, export, metadata, bounded provisioning/signing, screenshot sync and commerce planning implemented; not published.** A source build exposes offline validation, app discovery, exact-version state reads, safe local export, manual app-record preparation, documentation and eight JSON Schemas. Headroom discovery/export, four-locale metadata synchronization, free pricing and configured territory availability have been verified; selected localized text, shared metadata, bundle registration, screenshots and simple base prices use approved plans. The remaining first-release features below are not yet available.
...
The shared plan engine is implemented, with immutable process-bound plans, exact-subset host approval, flushed redacted journals, stale-state checks and readback reconciliation. Adapters cover selected localized/shared metadata, screenshot sets, bundle IDs, a bounded capability subset, explicit signing resources and simple app-wide base prices. Availability comparison is read-only and reports manual actions because generic public writes are not established. Submission remains pending. See [plan and recovery semantics](Documentation/Plan-and-Apply.md), [signing boundaries](Documentation/Signing.md) and [commerce boundaries](Documentation/Commerce.md).
