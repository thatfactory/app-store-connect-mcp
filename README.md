...
The shared plan engine is implemented, with immutable process-bound plans, exact-subset host approval, flushed redacted journals, stale-state checks and readback reconciliation. Adapters cover selected localized/shared metadata, screenshot sets, bundle IDs, a bounded capability subset and simple app-wide base prices. Availability comparison is read-only and reports manual actions because generic public writes are not established. Submission remains pending. See [plan and recovery semantics](Documentation/Plan-and-Apply.md) and [commerce boundaries](Documentation/Commerce.md).
