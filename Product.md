...
## 2. Users and first customer

The primary user is an Apple-platform developer working with an MCP-capable coding agent and a local repository checkout. The initial real application is Headroom: a native macOS menu bar utility with English (US), German, French, Japanese, and Brazilian Portuguese localizations.

Headroom already has an App Store record and manually entered English metadata. Adoption must begin by reading that record, preserving the existing English values, and adding the other locales. The product must not require rebuilding an existing listing from scratch.
...
Headroom's four additional locale listings are applied from Git-owned content without overwriting English. Each configured locale has the intended ordered screenshots in the selected draft version. The second full synchronization makes zero remote mutations. Interruption recovery does not duplicate assets or versions.
...
Successful metadata dogfooding is not evidence of successful submission. Record live-verification coverage separately for each domain. A release cannot claim a tested operation without evidence from its corresponding acceptance path.
...
| Future | IAPs and subscriptions | Products, localized purchase metadata, pricing, availability, review assets, and submission. Headroom's future non-consumable unlock is the natural dogfood. |
...
Do not extract a shared npm core with `xcode-cloud-mcp` before the two implementations have proven common requirements. Reusing a small amount of attributed code initially is simpler than imposing coordinated releases on both products. Review shared-library extraction after the first real release.
