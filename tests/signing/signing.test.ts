...
const certificate=(id:string,type='IOS_DEVELOPMENT',expiration='2999-01-01T00:00:00Z')=>({id,type:'certificates',attributes:{name:'Development certificate',certificateType:type,platform:'IOS',expirationDate:expiration,activated:true,serialNumber:'01'}});
...
async function fixture(t:test.TestContext){
  ...
  const key=path.join(checkout,'test-key.pem');const csrPath=path.join(root,'request.csr');execFileSync('/usr/bin/openssl',['req','-new','-newkey','rsa:2048','-nodes','-subj','/CN=Test Signing','-keyout',key,'-out',csrPath],{stdio:'ignore'});
  const state={certificates:[certificate('CERT')],devices:[device('DEVICE')],profiles:[profile('PROFILE')],unknown:false,forbidden:false,accountLimit:false,concurrentExtra:false};const calls:{method:string;url:string;body?:any}[]=[];
  const api=new ApiClient({token:()=> 'api-secret'},{readRetries:0,fetch:async(input,options)=>{
    ...
    if(method==='POST'&&url.pathname==='/v1/certificates')state.certificates.push(certificate('NEW-CERT',body.data.attributes.certificateType));
...
test('explicit certificate revocation exposes affected profiles and never creates replacement',native,async t=>{
  const f=await fixture(t);const p=await f.plan({type:'revokeCertificate',certificateId:'CERT'});assert.equal((p.value.summary as any).affectedProfileCount,1);assert.equal((await p.apply()).state,'complete');assert.equal(f.state.certificates.length,0);assert.equal(f.state.profiles.length,1);assert.equal(f.state.profiles[0]!.attributes.profileState,'INVALID');assert.equal(f.calls.filter(call=>call.method==='DELETE').length,1);assert.equal(f.calls.filter(call=>call.method==='POST').length,0);
});
...
test('profile creation requires exact compatible relationships and does not attach visible extras',native,async t=>{
  const f=await fixture(t);f.state.devices.push(device('UNSELECTED','00008110-008888888888001E'));const p=await f.plan({type:'createProfile',name:'Exact profile',profileType:'IOS_APP_DEVELOPMENT',bundleId:'BUNDLE',certificateIds:['CERT'],deviceIds:['DEVICE']});assert.equal((p.value.summary as any).deviceCount,1);assert.equal((await p.apply()).state,'complete');assert.deepEqual(f.state.profiles.at(-1)!.relationships.devices.data,[{type:'devices',id:'DEVICE'}]);
